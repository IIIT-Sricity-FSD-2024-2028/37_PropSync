...
function getParticipants() {
  ...
  if (saved) {
    try {
      ...
      const hasCurrentSeed = parsed.some(p => p.email === 'admin.primary@propsync.com') &&
...
const INITIAL_ROLES = [
  {
    id: 1, name: 'Owner', userCount: '456 users',
    description: 'Property owners who report and track maintenance issues',
    iconType: 'home', bgClass: 'bg-blue-100', textClass: 'text-blue-600', borderClass: 'owner',
    permissions: [
      'Report maintenance issues', 'Track complaint progress',
      'Confirm work completion', 'Rate service quality', 'Receive status notifications',
    ],
  },
  {
    id: 2, name: 'Maintenance Manager', userCount: '89 users',
    description: 'Managers who review complaints and coordinate service providers',
    iconType: 'wrench', bgClass: 'bg-green-100', textClass: 'text-green-600', borderClass: 'manager',
    permissions: [
      'Review maintenance complaints', 'Assign service providers',
      'Reassign complaints', 'Monitor work progress', 'Approve/reject estimates', 'Pay service bills',
    ],
  },
  {
    id: 3, name: 'Service Provider', userCount: '703 users',
    description: 'External contractors who perform maintenance services',
    iconType: 'briefcase', bgClass: 'bg-orange-100', textClass: 'text-orange-600', borderClass: 'provider',
    permissions: [
      'Access assigned tasks', 'Accept/reject tasks', 'Perform maintenance services',
      'Send service estimates', 'Mark tasks completed', 'Submit service bills', 'Review ratings & feedback',
    ],
  },
];
...
function getComplaints() {
  ...
  if (saved) {
    try {
      const parsed = JSON.parse(saved);
      const hasOldSeed = parsed.some(c => c.id === 'CPL001' || c.reportedBy === 'Sarah Mitchell');
...
const INITIAL_NOTIFICATIONS = [
  ...
  { id:5, title:'Role Permissions Updated',       category:'Security',             description:"The 'Maintenance Manager' role permissions have been successfully updated with new access controls.",                       timestamp:'1 day ago',   isNew:false, isRead:false, type:'success', iconType:'shield' },
  { id:6, title:'Database Backup Completed',      category:'System',               description:'Automatic database backup completed successfully. Backup size: 2.4 GB. All data has been securely stored.',               timestamp:'1 day ago',   isNew:false, isRead:true,  type:'success', iconType:'check' },
  { id:7, title:'Configuration Changes Detected', category:'System Configuration', description:'Email notification settings have been modified by administrator. Review changes in System Configuration.',                 timestamp:'2 days ago',  isNew:false, isRead:true,  type:'info',    iconType:'settings' },
];
...
function getHiddenBackendNotificationIds() {
  try {
    return JSON.parse(localStorage.getItem('hiddenBackendAdminNotifications')) || [];
  } catch {
    return [];
  }
}
...
function hideBackendNotification(backendId) {
  ...
  if (!ids.includes(id)) {
    ids.push(id);
    localStorage.setItem('hiddenBackendAdminNotifications', JSON.stringify(ids));
  }
}

function getAcceptedBackendNotificationIds() {
  try {
    return JSON.parse(localStorage.getItem('acceptedBackendAdminNotifications')) || [];
  } catch {
    return [];
  }
}
...
function acceptBackendNotification(backendId) {
  ...
  if (!ids.includes(id)) {
    ids.push(id);
    localStorage.setItem('acceptedBackendAdminNotifications', JSON.stringify(ids));
  }
}
