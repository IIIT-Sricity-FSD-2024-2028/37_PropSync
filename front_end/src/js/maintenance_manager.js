...
function rejectComplaintById(id, reason) {
  ...
  if (c) {
    ...
    saveComplaints(data);
    addNotification('cross', '#FEE2E2', 'Complaint Rejected', `Complaint ${id} rejected. Rejection reason sent to resident.`, 'all', false);
  }
}
...
function getNotifications() {
  ...
  const defaults = [
    ...
    {id:3,icon:'checkmark',color:'#DCFCE7',title:'Work completed',desc:'Electrical repair completed in Tower B, Apt 304 (C-2395)',time:'2 hrs ago',unread:false,recipient:'all',userCreated:false},
    ...
    {id:7,icon:'money',color:'#F0FDF4',title:'Payment processed',desc:'Payment of ₹4500 processed for C-2385',time:'2 days ago',unread:false,recipient:'all',userCreated:false},
    {id:8,icon:'clipboard',color:'#F0FDF4',title:'Complaint awaiting approval',desc:'Complaint C-2410 needs your review',time:'2 days ago',unread:false,recipient:'owner',userCreated:false}
  ];
...
function createCustomNotification(title, desc, recipient, icon, color) {
  addNotification(icon || 'bell', color || '#DCFCE7', title, desc, recipient, true);
  showToast(`✅ Notification sent to ${recipientLabel(recipient)}.`);
}

function recipientLabel(r) {
  const map = {all:'All Users', owner:'Property Owner', resident:'Resident', provider:'Service Provider'};
  return map[r] || r;
}
...
/* ============================================================
   ...
   ============================================================ */
const iconMap = {
  drop: '💧', chart: '📊', checkmark: '✅', warn: '⚠️',
  wrench: '🔧', cross: '❌', money: '💰', clipboard: '📋',
  bell: '🔔', person: '👤'
};
...
/* ============================================================
   SIDEBAR – hamburger opens, close button closes
   ============================================================ */
...
function markNotifRead(id) {
  ...
  if (n && n.unread) { n.unread = false; saveNotifications(notifs); updateNotifDot(); }
