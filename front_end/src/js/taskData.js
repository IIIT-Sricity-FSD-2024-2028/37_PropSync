/* ─────────────────────────────────────────────
   STORAGE KEYS
───────────────────────────────────────────── */
const PROFILE_VERSION_KEY = "propSyncProfileVersion";
...
function getProfileKey() {
  ...
  return user.email ? `${PROFILE_KEY}:${user.email}` : PROFILE_KEY;
}

function getInitialProfile() {
  const user = getCurrentUser();
  const name = user.name || "QuickFix Plumbing";
  ...
  return {
    ...
    locations: user.communityName || "Green Valley Society",
    spId: `SP-${String(user.id || 9).padStart(4, "0")}`,
  };
}
...
(function _initStorage() {
  if (localStorage.getItem(PROFILE_VERSION_KEY) !== CURRENT_VERSION) {
    localStorage.setItem(getProfileKey(), JSON.stringify(initialProfile));
    localStorage.setItem(NOTIFS_KEY, JSON.stringify(initialNotifications));
    localStorage.setItem(PROFILE_VERSION_KEY, CURRENT_VERSION);
  } else if (!localStorage.getItem(getProfileKey())) {
    localStorage.setItem(getProfileKey(), JSON.stringify(initialProfile));
  }
})();
